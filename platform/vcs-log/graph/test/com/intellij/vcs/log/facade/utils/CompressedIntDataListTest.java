...
public class CompressedIntDataListTest {
  ...
  private static String toStr(@NotNull IntDataList intList) {
  ...
  private static IntDataList toDataList(final int ... list) {
    return new IntDataList() {
      @Override
      public int size() {
        return list.length;
      }
  ...
  protected void runTest(int ... list) {
    IntDataList intDataList = CompressedIntDataList.newInstance(list, 3);
    String expected = toStr(toDataList(list));
    assertEquals(expected, toStr(intDataList));
  }
