  ...
  protected List<ShortEdge> getDownShortEdges(int rowIndex) {
    ...
    for (int startPosition = 0; startPosition < visibleElements.size(); startPosition++) {
      ...
      if (element instanceof GraphNode) {
        for (GraphEdge edge : myPrintedLinearGraph.getDownEdges(((GraphNode)element).getNodeIndex())) {
  ...
  private Function<GraphEdge, Integer> createEndPositionFunction(int visibleRowIndex) {
    ...
    return new Function<GraphEdge, Integer>() {
      ...
      public Integer fun(GraphEdge edge) {
        ...
        if (position == null && edge.getType() != GraphEdgeType.NOT_LOAD_COMMIT) {
          position = toPosition.get(new GraphNode(edge.getDownNodeIndex())); // todo very strange code! FIX THIS!!
        }
    ...
    for (GraphElement element : getSortedVisibleElementsInRow(visibleRowIndex)) {
      ...
      if (element instanceof GraphEdge) {
        ...
        if (edge.getType() == GraphEdgeType.NOT_LOAD_COMMIT) {
          edgeSize = Integer.MAX_VALUE;
          downOffset = Integer.MAX_VALUE;
        } else {
          edgeSize = edge.getDownNodeIndex() - edge.getUpNodeIndex();
          downOffset = edge.getDownNodeIndex() - visibleRowIndex;
        }
  ...
  private class GraphElementComparator implements Comparator<GraphElement> {
    ...
    public int compare(@NotNull GraphElement o1, @NotNull GraphElement o2) {
      if (o1 instanceof GraphEdge && o2 instanceof GraphEdge) {
        ...
        int upNodeIndex1 = edge1.getUpNodeIndex();
        int upNodeIndex2 = edge2.getUpNodeIndex();

        if (upNodeIndex1 == upNodeIndex2) {
          if (edge1.getType() == GraphEdgeType.NOT_LOAD_COMMIT && edge2.getType() == GraphEdgeType.NOT_LOAD_COMMIT)
            return 0;

          if (edge1.getType() == GraphEdgeType.NOT_LOAD_COMMIT)
            return 1;

          if (edge2.getType() == GraphEdgeType.NOT_LOAD_COMMIT)
            return -1;
